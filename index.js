...
function init(){
    totalSeconds = Math.floor((new Date('09.09.2022') - new Date())/1000);
    setTimeLeft();
    let interval= setInterval(() =>{
        if (totalSeconds < 0){
            clearInterval(interval);
            document.getElementsByClassName('days')=00;
            document.getElementsByClassName('minutes')=00;
            document.getElementsByClassName('hours')=00;
            document.getElementsByClassName('seconds')=00;
            document.getElementsByClassName('title')='Rocket has launched';
...
function printTime(){
   ...
   // Had help with the animation.

   animatedFlip(days,timeLeft.d);
   animatedFlip(hours,timeLeft.h);
   animatedFlip(minutes,timeLeft.m);
   animatedFlip(seconds,timeLeft.s);
}

function animatedFlip(element,value){
